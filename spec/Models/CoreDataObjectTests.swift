...
class CoreDataObjectTests: XCTestCase
{
    ...
    func testObjectQuerying()
    {
        ...
        XCTAssertEqual(CoreDataObject.sorted(by: "name", ascending: true).first().find().firstObject().name, "Realmy", "The name of the first object returned should be Realmy")
        XCTAssertEqual(CoreDataObject.sorted(by: "name", ascending: true).last().find().firstObject().name, "Realmy2", "The name of the first object returned should be Realmy2")
